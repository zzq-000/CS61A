...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            if isinstance(place.ant,ContainerAnt) and not place.ant.contained_ant\
                and not isinstance(self,ContainerAnt):
                    place.ant.contained_ant = self
            elif isinstance(self,ContainerAnt) and not self.contained_ant \
                and not isinstance(place.ant,ContainerAnt):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while ptr is not beehive:
            if self.min_range <= step <= self.max_range and len(ptr.bees) !=0:
                return rANTdom_else_none(ptr.bees) # REPLACE THIS LINE
            ptr = ptr.entrance
            step += 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.place and len(self.place.bees) > 0:
            ...
            for i in range(len(copy_bees)):
                Insect.reduce_armor(copy_bees[i],amount + delta)
            # print(copy_bees)
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if not self.is_real:
            ...
            while ptr is not None:
                if ptr.ant:
                    if (not hasattr(ptr.ant,'damage_doubled') or\
                        not ptr.ant.damage_doubled):
                        ptr.ant.damage *= 2
                        ptr.ant.damage_doubled = True
                    if isinstance(ptr.ant,ContainerAnt):
                        if ptr.ant.contained_ant and (not hasattr(ptr.ant.contained_ant,'damage_doubled') or\
                    not ptr.ant.contained_ant.damage_doubled):
                            ptr.ant.contained_ant.damage *= 2
                            ptr.ant.contained_ant.damage_doubled = True
                ptr = ptr.exit
    ...
    def reduce_armor(self, amount):
        ...
        # END Problem 13
    def remove_from(self, place):
        if not isinstance(self,QueenAnt) or not self.is_real:
            Ant.remove_from(self,place)     
...
def apply_status(status, bee, length):
    ...
    def counter(old,new):
        ...
        action.count = 0
        return action
    bee.action = counter(orginal,new_action)
    # def counter(f):
    #     def action(*args):
    #         action.count += 1
    #         return f(*args)
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target and (not hasattr(target,'has_scared') or not target.has_scared):
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        if self.place.ant != self:
            table[self.place.ant] = distance
        if len(self.place.bees) > 0:
            for i in self.place.bees:
                table[i] = distance
        ptr = self.place.entrance
        distance += 1
        while ptr is not beehive:
            if ptr.ant:
                table[ptr.ant] = distance
                if isinstance(ptr.ant,ContainerAnt) and ptr.ant.contained_ant:
                    table[ptr.ant.contained_ant] = distance
            if len(ptr.bees) > 0:
                for bee in ptr.bees:
                    table[bee] = distance
            ptr = ptr.entrance
