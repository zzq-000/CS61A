...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    """
    # Evaluate atoms
    # print("Debug:", expr,(type(expr)),self_evaluating(expr))
...
def eval_all(expressions, env):
    ...
    while ptr is not nil:
        result = scheme_eval(ptr.first,env)
        ptr = ptr.rest
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        while fp is not None:
            if symbol in fp.bindings:
                return fp.bindings[symbol]
            fp = fp.parent
        # END PROBLEM 2
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('arguments not match')
        child_frame = Frame(self)
        ptr_for = formals
        ptr_val = vals
        while ptr_for is not nil:
            child_frame.define(ptr_for.first, ptr_val.first)
            ptr_for = ptr_for.rest
            ptr_val = ptr_val.rest
        return child_frame
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        f = Frame.make_child_frame(self.env,self.formals,args)
        return f
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        # print("Deubg:",expressions)
        validate_form(expressions, 2, 2) # Checks that expressions is a list of length exactly 2
        # BEGIN PROBLEM 5
        rest = expressions.rest
        value = rest.map(lambda x:scheme_eval(x, env))
        # value = scheme_eval(rest,env)
        # target = scheme_eval(target,env)
        env.define(target, value.first)
        return target
        # END PROBLEM 5
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    # print("Deubg:",bindings)
    # validate_formals(bindings)
    while bindings is not nil:
        validate_form(bindings.first,2,2)
        # print("Debug:",bindings.first,bindings.first.rest)
        names = Pair(bindings.first.first,names)
        val = bindings.first.rest.map(lambda x:scheme_eval(x,env)).first
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        of values, for a lexically-scoped call evaluated in my parent environment."""
        # BEGIN PROBLEM 11
        f = Frame.make_child_frame(env,self.formals,args)
        return f
    # END PROBLEM 15
