...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        edits,cost = '+['+code_char+']'+edits,cost+1
        possibilities.append((edits, cost))
        # Delete
        edits, cost = helper_align(skeleton_idx+1,code_idx)
        edits, cost = '-['+skel_char+']'+edits,cost+1
        possibilities.append((edits, cost))
...
def insert(link, value, index):
    ...
    IndexError
    """
    if index > 0 and link is Link.empty:
        raise IndexError
    elif index == 0:
        print("Debug:",value)
        link.first = value
        ...
        link = Link(link.first,insert(link.rest,value,index-1))
