...
def add_chars(w1, w2):
    ...
    True
    """
    def helper_get_index(char, s):
        if char == s[0]:
            return 0
        else:
            return 1 + helper_get_index(char, s[1:])
    
    if w1 == '':
        return w2
    
    index = helper_get_index(w1[0], w2)
    return add_chars(w1[1:], w2[0:index]+w2[index + 1:])
