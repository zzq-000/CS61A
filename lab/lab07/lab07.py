def insert_into_all(item, nested_list):
    ...
    [[0], [0, 1, 2], [0, 3]]
    """
    return [(lambda x: [item] + x[:])(i) for i in nested_list]
...
def subseqs(s):
    ...
    if len(s)==0:
        ...
        partitial = subseqs(s[1:])
        return partitial + insert_into_all(s[0],partitial)
...
def trade(first, second):
    ...
    while not equal_prefix() and m <= len(second) and n <= len(second):
...
def make_glookup(class_assignments):
    ...
    def calculate(assignment,score):
        nonlocal input_total_score,earned_score
        input_total_score += class_assignments[assignment]
        earned_score += score
        return earned_score / input_total_score
    return calculate
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n == 1 or n == 2:
        return 1
    return  sum([num_trees(i + 1) * num_trees(n - i - 1) * (2 if i + 1 != n - i - 1 else 1) for i in range(n // 2)])
