...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for i in player.deck.cards:
            i.attack,i.defense = i.attack + other_card.attack, i.defense + other_card.defense
        left = []
        while not opponent.deck.is_empty():
            pop_card = opponent.deck.draw()
            if pop_card.attack != other_card.attack and pop_card.defense != other_card.defense:
                left.append(pop_card)
        opponent.deck = Deck([ card.copy() for card in left])
