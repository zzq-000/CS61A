...
def balanced(m):
    ...
    left_arm = left(m)
    right_arm = right(m)
    if length(left_arm) * total_weight(end(left_arm)) == \
...
def totals_tree(m):
    ...
    if is_planet(m):
        return tree(size(m))
    return tree(total_weight(m),branches = [totals_tree(end(left(m))),\
...
def sub_interval(x, y):
    ...
    and any value in y."""
    return add_interval(x,mul_interval(y,interval(-1,-1)))
...
def quadratic(x, a, b, c):
    ...
    right_v = f(right)
    max_quad = f(extreme)
    if left <= extreme <= right:
        return interval(min(left_v,right_v,max_quad),max(left_v, right_v,max_quad))
    return interval(min(left_v,right_v),max(left_v, right_v))
