...
class Coin:
    ...
    def worth(self):
        if Mint.current_year - self.year > 50:
            return type(self).cents + (Mint.current_year - self.year - 50)
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        min_val = bst_min(t.branches[1])
        max_val = bst_max(t.branches[0])
        return max_val<= t.label <= min_val and is_bst(t.branches[0]) and is_bst(t.branches[1])
