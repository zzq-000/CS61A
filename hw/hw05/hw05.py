...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, passwd):
        nonlocal incorrect_passwds,balance
        if len(incorrect_passwds) == 3:
                return f"Too many incorrect attempts. Attempts: ['{incorrect_passwds[0]}', '{incorrect_passwds[1]}', '{incorrect_passwds[2]}']"
        else:
            if passwd == password:
                if balance > amount:
                    balance -= amount
                ...
                incorrect_passwds.append(passwd)
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    "Too many incorrect attempts. Attempts: ['my', 'secret', 'password']"
    """
    valid = withdraw(0,old_pass)
    if type(valid) == str:
        return valid
    def joint(amount, passwd):
        if passwd == new_pass or passwd == old_pass:
...
def remainders_generator(m):
    ...
    11
    """
    def inner_generation(remainder):
